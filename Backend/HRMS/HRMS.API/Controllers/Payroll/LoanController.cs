...
using HRMS.Application.Features.Payroll.Loans.Queries.GetAllLoans;
...
using HRMS.Application.Features.Payroll.Loans.Queries.GetPendingLoans;
...
public class LoanController : ControllerBase
{
    ...
    /// </summary>
    [HttpGet("all")]
    public async Task<ActionResult<Result<List<LoanDto>>>> GetAllLoans(
    ...
    [HttpPost("settle/{loanId}")]
    public async Task<ActionResult<Result<bool>>> SettleLoan(int loanId)
    {
        var result = await _mediator.Send(new EarlySettlementCommand { LoanId = loanId });
        return result.Succeeded ? Ok(result) : BadRequest(result);
    }
    ...
    public async Task<ActionResult<Result<List<LoanDto>>>> GetPendingLoans([FromQuery] int? departmentId)
    {
        var query = new GetPendingLoansQuery { DepartmentId = departmentId };
        var result = await _mediator.Send(query);
        return Ok(result);
    }
}
