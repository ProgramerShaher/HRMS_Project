...
/// <summary>
/// محقق صحة بيانات أمر تحديث نوع الوثيقة
/// </summary>
public class UpdateDocumentTypeCommandValidator : AbstractValidator<UpdateDocumentTypeCommand>
{
    private readonly IApplicationDbContext _context;

    public UpdateDocumentTypeCommandValidator(IApplicationDbContext context)
    {
        _context = context;

        RuleFor(x => x.DocumentTypeId)
            .GreaterThan(0).WithMessage("معرف نوع الوثيقة غير صحيح");
    ...
    private async Task<bool> BeUniqueNameAr(UpdateDocumentTypeCommand command, string nameAr, CancellationToken cancellationToken)
    {
        return !await _context.DocumentTypes.AnyAsync(
            d => d.DocumentTypeNameAr == nameAr && d.DocumentTypeId != command.DocumentTypeId, 
            cancellationToken);
    }
